$(document).ready(function() {
    ...
    $.post("../MCC/Resources/PHP/GetJOStat.php",
    function(data, status){
        ...
        $(".act").find(".ViewSR").click(function () {
            ...
            var prop=JSON.stringify(
              ...
              $.post("../MCC/Resources/php/viewsignature.php",prop,
              function(data, status){
                var data=$.parseJSON(data); 
              ...
              $.post("../MCC/Resources/php/viewsignature2.php",prop,
              function(data, status){
                var data=$.parseJSON(data); 
              ...
              $.post("../MCC/Resources/PHP/SRDets.php",prop,
                  function(data, status){
                  var data=$.parseJSON(data);    
                  var row="";
                  for(var i=0;i<data.length;i++){
    ...
    $.post("../MCC/Resources/PHP/GetDesig.php",
    function(data, status){
        var data=$.parseJSON(data);    
        var row="";
    ...
    $('#JO_User').on('change', function() {
        $('#JO_Type').val("");
        $('#JO_PropertyNo').val("");
        $('#JO_ItemID').val("");
        $('#JO_Model').val("");
        $("#JO_Serial").empty().append("<option value='--SELECT--'>--SELECT--</option>");
        ...
        $.post("../MCC/Resources/PHP/GetDesigSerial.php",json,
        function(data, status){
            var data=$.parseJSON(data);    
            var row="";
            ...
            for(var i=0;i<data.length;i++){
                ...
                $("#JO_Serial").append("<option value='"+id+"'>"+name+"</option>");
            }  
        });
    });

    $('#JO_Serial').on('change', function() {
        ...
        $.post("../MCC/Resources/PHP/GetDesigSerialStat.php",json,
        function(data, status){
            var data=$.parseJSON(data);    
            var row="";
            ...
            for(var i=0;i<data.length;i++){
                ...
                var WhatType = data[i].WhatType;
                var Property = data[i].Property;
                var PropNo = data[i].PropNo;
                var model = data[i].model

                $('#JO_Type').val(WhatType);
                $('#JO_PropertyNo').val(Property);
                $('#JO_ItemID').val(PropNo);
                $('#JO_Model').val(model);
            }  
        });
    });
    ...
    $("#JO_Save").on('click', function () {
        var JO_Serial=$("#JO_Serial").get(0).value;
        var JO_Desc=$("#JO_Desc").get(0).value;
        var JO_Desti = $("#JO_Desti").get(0).value;
        var JO_Type = $("#JO_Type").get(0).value;
        var JO_PropertyNo = $("#JO_PropertyNo").get(0).value;
        var JO_ItemID = $("#JO_ItemID").get(0).value;
        var JO_Model = $("#JO_Model").get(0).value;
        ...
        if(JO_Serial != "--SELECT--" && JO_Desti != "--SELECT--" && JO_Desc != "")
        {
            alert("Please Wait");
            $('#JO_Save').prop('disabled', true);

            $.post("../MCC/Resources/PHP/AddJO.php",prop,
            function(data,status){
                if(data == 0) 
                { 
                    alert("ERROR");
    ...
    $("#save_cancel").on('click', function () {
        ...
        {
            alert("Please Wait");
            $.post("../MCC/Resources/PHP/CancelJO.php",prop,
            function(data,status){
                if(data == 1) 
                { 
