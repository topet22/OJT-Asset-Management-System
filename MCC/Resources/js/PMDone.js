$(document).ready(function() {
    ...
    $.post("../MCC/Resources/PHP/PMDone.php",
    ...
    for(var i=0;i<data.length;i++){
        ...
        if(data[i].PM_STATUS == "APPROVED"){
            row=row+"<tr><td>"+data[i].PM_Department+"</td><td>"+data[i].PM_DesigUser+"</td><td>"+data[i].PM_SerialPC+"</td><td><span class ='badge rounded-pill bg-success'>"
            +data[i].PM_STATUS+"</span></td><td class='jo_act'><button id="+data[i].PM_ID+" class='View  btn btn-outline-dark' data-bs-toggle='modal' data-bs-target='#tres'><i class='bi bi-file-earmark-plus'></i>&nbsp;View PM Checklist</button></td></tr>";
        }
        else{
            row=row+"<tr><td>"+data[i].PM_Department+"</td><td>"+data[i].PM_DesigUser+"</td><td>"+data[i].PM_SerialPC+"</td><td><span class ='badge rounded-pill bg-primary'>"
            +data[i].PM_STATUS+"</span></td><td class='jo_act'><button id="+data[i].PM_ID+" class='View  btn btn-outline-dark' data-bs-toggle='modal' data-bs-target='#tres'><i class='bi bi-file-earmark-plus'></i>&nbsp;View PM Checklist</button></td></tr>";
        }
    ...
    $(".jo_act").find(".View").click(function () {
        ...
        var prop=JSON.stringify(
           ...
           $.post("../MCC/Resources/PHP/PMDets.php",prop,
           function(data, status){
            var data=$.parseJSON(data);    
            var row="";
            for(var i=0;i<data.length;i++){
          ...
          $.post("../MCC/Resources/PHP/viewpmsignature.php",prop,
          function(data, status){
            var data=$.parseJSON(data); 
...
$("#verify").on('click', function () {
    ...
    var prop=JSON.stringify(
      ...
      $.post("../MCC/Resources/PHP/verifyPM.php",prop,
      function(data, status){
          if(data == 1) 
          { 
              alert("PM Verified!");
