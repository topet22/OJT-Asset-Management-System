$(document).ready(function() {
    ...
    $.post("../SuperAdmin/Resources/PHP/viewdepusers.php",
    function(data, status){
        var data=$.parseJSON(data);    
        var row="";
        for(var i=0;i<data.length;i++){
        ...
        if(WhatType != "--SELECT--" && InvSerial!="" && InvModel!="" && InvBrand!="")
        {
            ...
            if(WhatType == "Computer")
            {
                if(CompOS != "" && Comptype!="--SELECT--" && CompStorageType!="--SELECT--" && CompRAM!="" && CompProcessor!="" && OSLicense!="" ){
                    $.post("../SuperAdmin/Resources/PHP/SupAdminAddStock.php",
                    JSON.stringify({
                        WhatType : WhatType,
                        InvPropNo:InvPropNo,
                        InvSerial:InvSerial,
    ...
    $('#searchwhat').on('change', function() {
        var selected = $(this).val();
        if(selected == "Computer"){
            $.post("../SuperAdmin/Resources/PHP/SupAdminComputerStocks.php",
            function(data, status){
            var data=$.parseJSON(data);    
            var row="";
            for(var i=0;i<data.length;i++){
            ...
            $.post("../SuperAdmin/Resources/PHP/SupAdminPrinterStocks.php",
            function(data, status){
            var data=$.parseJSON(data);    
            var row="";
            for(var i=0;i<data.length;i++){
            ...
            $.post("../SuperAdmin/Resources/PHP/SupAdminOthersStock.php",
            function(data, status){
            var data=$.parseJSON(data);    
            var row="";
            for(var i=0;i<data.length;i++){
    ...
    $('#DepartmentGoingTo').on('change', function() {
        ...
        if(selected == "--SELECT--"){
            ...
            $.post("../SuperAdmin/Resources/PHP/viewdepusers.php",sel,
            function(data, status){
                var data=$.parseJSON(data);    
                var row="";
                for(var i=0;i<data.length;i++){
    ...
    $("#TransferStock").on('click', function (){
        ...
        if (dept !== "--SELECT--" && desiguser !== "--SELECT--"){
            ...
            if (confirm("Do you want to transfer this Item?")) {
                $.post("../SuperAdmin/Resources/PHP/SupAdminTransferStock.php",prop,
                function(data, status){
                    if(data == 1) 
                    { 
    ...
    $("#save_user").on('click', function () {
        ...
        if(AddUsername!="" && AddFullName!="" && AddPassword!="" && ConfirmAddPassword!="" && UserDepartment!="" && AddUserLevel   !="")
        {
