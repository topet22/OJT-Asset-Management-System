$(document).ready(function() {
    ...
    $.post("../Admin/Resources/PHP/SupAdminGetNum.php",
    ...
    for(var i=0;i<data.length;i++){
        ...
        // 
        var numPO = data[i].InvPO;
        var numFR = data[i].InvFR;
        var numCondemn = data[i].InvCondemn;
        var numServiceable = data[i].InvService;

        $("#NumberOfComputer").html(numcomp);
        $("#NumberOfPrinter").html(numprt);
        $("#NumberOfOthers").html(numoth);

        $("#NumberServiceable").html(numServiceable);
        $("#NumberFR").html(numFR);
        $("#NumberPO").html(numPO);
    ...
    $('#searchwhat').on('change', function() {
        ...
        if(selected == "Computer"){
            ...
            table_comp1.destroy();
            $.post("../Admin/Resources/PHP/SupAdminComputers.php",
            function(data, status){
            var data=$.parseJSON(data);    
            var row="";
            for(var i=0;i<data.length;i++){
                ...
                row=row+"<tr><td>"+data[i].InvUser+"</td><td>"+data[i].ItemID+"</td><td>"
                +data[i].InvPropNo+"</td><td>"
                +data[i].InvSerial+"</td><td>"
                +data[i].InvDate+"</td><td class='comp_vew'><button id="+data[i].comp_ID+" class='View btn btn-info mb-2' data-bs-toggle='modal' data-bs-target='#view'><i class='fa fa-eye link-dark'></i>&nbsp;View Details</button></td></tr>";
            }
            $('#comp_data').html(row);
            $(".comp_vew").find(".View").click(function () {
                ...
                $.post("../Admin/Resources/PHP/SupAdminViewEdit.php",prop,
                function(data, status){
                    var data=$.parseJSON(data); 
                    $('#desc').hide();
                    $('#sel_comp').show();
            ...
            $(".comp_vew").find(".delete").click(function () {
                ...
                if (confirm("Do you Want to Delete this record?")) {
                    $.post("../Admin/Resources/PHP/SupAdminDelete.php",prop,
                    function(data, status){
                        if(data == 1) 
                        { 
            ...
            $.post("../Admin/Resources/PHP/SupAdminPrinters.php",
            function(data, status){
            var data=$.parseJSON(data);    
            var row="";
            for(var i=0;i<data.length;i++){
                ...
                +data[i].InvPropNo+"</td><td>"
                +data[i].InvSerial+"</td><td>"
                +data[i].InvDate+"</td><td class='comp_vew'><button id="+data[i].printer_ID+" class='View btn btn-info mb-2' data-bs-toggle='modal' data-bs-target='#view'><i class='fa fa-eye link-dark'></i>&nbsp;View Details</button></td></tr>";
            }
            $('#prt_data').html(row);
            $(".comp_vew").find(".View").click(function () {
                var propID = this.id;
            ...
            $.post("../Admin/Resources/PHP/SupAdminOthers.php",
            function(data, status){
            var data=$.parseJSON(data);    
            var row="";
            for(var i=0;i<data.length;i++){
                ...
                +data[i].InvPropNo+"</td><td>"
                +data[i].InvSerial+"</td><td>"
                +data[i].InvDate+"</td><td class='comp_vew'><button id="+data[i].other_ID+" class='View btn btn-info mb-2' data-bs-toggle='modal' data-bs-target='#view'><i class='fa fa-eye link-dark'></i>&nbsp;View Details</button></td></tr>";
            }
            $('#etc_data').html(row);
            $(".comp_vew").find(".View").click(function () {
                var propID = this.id;
    ...
    $('#searchwhere').on('change', function() {
        ...
        if(selected == "Computer"){
            var table_comp1 = $('#tbl_computers').DataTable();
            table_comp1.destroy();
            $.post("../Admin/Resources/PHP/SupAdminDeptComputers.php",json,
            function(data, status){
            var data=$.parseJSON(data);    
            var row="";
            for(var i=0;i<data.length;i++){
            ...
            var table_print1 = $('#tbl_printers').DataTable();
            table_print1.destroy();
            $.post("../Admin/Resources/PHP/SupAdminDeptPrinters.php",json,
            function(data, status){
            var data=$.parseJSON(data);    
            var row="";
            for(var i=0;i<data.length;i++){
            ...
            var table_others1 = $('#tbl_others').DataTable();
            table_others1.destroy();
            $.post("../Admin/Resources/PHP/SupAdminDeptOthers.php",json,
            function(data, status){
            var data=$.parseJSON(data);    
            var row="";
            for(var i=0;i<data.length;i++){
    ...
    //JS for Selecting User Per Department Selected (START)

    // $('#InvDept').on('change', function() {
    //     $("#InvUser").empty();
    //     var selected = $(this).val();
    ...
    //                 $("#InvUser").append("<option value='"+id+"'>"+name+"</option>");
    ...
    function(data, status){
        ...
        for(var i=0;i<data.length;i++){
            ...
            $("#serviceDept").append("<option value='"+id+"'>"+name+"</option>");
        }
    });
        ...
        if(WhatType != "--SELECT--" && InvDept!="" && InvUser!="" &&  InvModel!="" && InvBrand!="")
        {
            ...
            if(WhatType == "Computer")
            {
                if(CompOS != "" && Comptype!="--SELECT--" && CompStorageType!="--SELECT--" && CompRAM!="" && CompProcessor!="" && OSLicense!="" ){
                    $.post("../Admin/Resources/PHP/addinventory.php",
                    JSON.stringify({
                        WhatType : WhatType,
                        InvDept:InvDept,
                        InvUser:InvUser,
    ...
    $('#searchtype').on('change', function() {
        ...
        if(selected == "Computer"){
            ...
            for(var i=0;i<data.length;i++){
                ...
                else if (status == "Pulled out/Repair Outside")
                {
                    row=row+"<tr><td>"+data[i].ItemID+"</td><td>"+data[i].InvPropNo+"</td><td>"
    ...
    $("#save_rems").on('click', function () {
        ...
        if(InvenStatus!="--SELECT--" && InvenReason!="")
        {
            ...
            $.post("../Admin/Resources/PHP/SupAdminAddRemarks.php",
            JSON.stringify({
    ...
    //
        ...
        $('#searcharchtype').on('change', function() {
            ...
            if(selected == "Computer")
            {
                ...
                table_comp1.destroy();
                $.post("../Admin/Resources/PHP/SupAdminArchComputers.php",
                function(data, status){
                var data=$.parseJSON(data);    
                var row="";
                for(var i=0;i<data.length;i++){
                ...
                table_print1.destroy();
                $.post("../Admin/Resources/PHP/SupAdminArchPrinters.php",
                function(data, status){
                var data=$.parseJSON(data);    
                var row="";
                for(var i=0;i<data.length;i++){
                ...
                table_others1.destroy();
                $.post("../Admin/Resources/PHP/SupAdminArchOthers.php",
                function(data, status){
                var data=$.parseJSON(data);    
                var row="";
                for(var i=0;i<data.length;i++){
        ...
        $('#searcharchdept').on('change', function() {
            ...
            var json =JSON.stringify(
                ...
                if(selected == "Computer"){
                    var table_comp1 = $('#tblarch_computers').DataTable();
                    table_comp1.destroy();
                    $.post("../Admin/Resources/PHP/SupAdminDeptArchComputers.php",json,
                    function(data, status){
                    var data=$.parseJSON(data);    
                    var row="";
                    for(var i=0;i<data.length;i++){
                ...
                var table_print1 = $('#tbl_printersarch').DataTable();
                table_print1.destroy();
                $.post("../Admin/Resources/PHP/SupAdminDeptArchPrinters.php",json,
                function(data, status){
                var data=$.parseJSON(data);    
                var row="";
                for(var i=0;i<data.length;i++){
                ...
                var table_others1 = $('#tbl_othersstat').DataTable();
                table_others1.destroy();
                $.post("../Admin/Resources/PHP/SupAdminDeptArchOthers.php",json,
                function(data, status){
                var data=$.parseJSON(data);    
                var row="";
                for(var i=0;i<data.length;i++)
    ...
    $.post("../Admin/Resources/PHP/SupAdminGetStockNum.php",
        function(data, status){
            var data=$.parseJSON(data);    
            var row="";
            for(var i=0;i<data.length;i++){
