...
$(document).ready(function() {
    ...
    $.post("../Admin/Resources/PHP/SupAdminPMDone.php",
    ...
    for(var i=0;i<data.length;i++){
        ...
        if (stats == "APPROVED")
        {
            row=row+"<tr><td>"+data[i].PM_DoneBy+"</td><td>"+data[i].PM_Department+"</td><td>"+data[i].PM_SerialPC+"</td><td><span class ='badge rounded-pill bg-success'>"
        ...
        {
            row=row+"<tr><td>"+data[i].PM_DoneBy+"</td><td>"+data[i].PM_Department+"</td><td>"+data[i].PM_SerialPC+"</td><td><span class ='badge rounded-pill bg-info'>"
            +data[i].PM_STATUS+"</span></td><td class='jo_act'><button id="+data[i].PM_ID+" class='View  btn btn-outline-dark' data-bs-toggle='modal' data-bs-target='#try'><i class='bi bi-file-earmark-plus'></i>&nbsp;View PM Checklist</button></td></tr>";
        }
    }
    $('#PM_data').html(row);
    ...
    $(".jo_act").find(".View").click(function () {
        ...
        var prop=JSON.stringify(
           ...
           $.post("../Admin/Resources/PHP/PMDets.php",prop,
           function(data, status){
            var data=$.parseJSON(data);    
            var row="";
            for(var i=0;i<data.length;i++){
                ...
                if(PM_STATUS == "APPROVED"){
                    $.post("../Admin/Resources/PHP/viewsignature2.php",
                    function(data, status){
                      var data=$.parseJSON(data); 
          ...
          $.post("../Admin/Resources/PHP/viewpmsignature.php",prop,
          function(data, status){
            var data=$.parseJSON(data); 
